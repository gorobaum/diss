...
\subsection{Estimativa da função de transformação}\index{Estimativa da função de transformação}
    ...
    Com o conjunto de pares de caracteristicas encontrado pelo passo anterior, os algoritmos de registro conseguem encontrar
um conjunto de parametros iniciais para sua transformação. Cada algoritmo representa a transformação necessária para
levar a imagem Alvo para a imagem Referência usando um modelo diferente, e utiliza métricas diferentes para passear
pelo espaço de paramêtros em busca do melhor resultado. Dada a grande diferença que os algoritmos mostram nesse passo 
em especifico, ele sera cobrido algoritmo por aloritmo na seção \ref{sec:algReg}.
...
dá o conjunto de parametros ótimo para a montagem da imagem final, logo esse passo tem como objetivo a aplicação
