...
desse passo depende totalmente da sua implementação.
    ...
    Podemos ainda utilizar o modelo \textit{MapReduce}, desenvolvido por \cite{dean2008mapreduce} e pela Google, para
acelerar esse passo e o anterior. O modelo \textit{MapReduce} foi desenvolvido para realizar processamento em lote de
um número gigantesco de dados, o popular \textit{Big Data}, dado que não exista muita diferença em como os dados
devem ser processados. Ele é composto de dois passos, o \textit{Map} onde os dados são pré-processados e mapeados para
grupos em comum; e o \textit{Reduce}, onde esses grupos são unidos de alguma maneira. O nosso \textit{Map} é a descoberta
das caracteristicas, e o \textit{Reduce} realiza a correspondencia entre elas.
