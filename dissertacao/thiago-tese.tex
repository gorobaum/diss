...
imagens de satélite. Deformações não rígidas representam melhor a ampla gama de possíveis 
deformações presentes nas várias categorias de imagens que são registradas. Atualmente, estudos
...
tempo de análise e processamento. Como as técnicas de registro não linear são tradicionalmente custosas
computacionalmente, o uso de \textit{graphic processing units} (GPUs) possibilita a
aceleração das diversas etapas do processo registro via hardware, tais como a extração de características,
determinação de correspondências e cálculo das funções de deformação. Este trabalho
tem como objetivo o estudo e a paralelização de técnicas de registro não linear aplicadas à arquiteturas modernas de 
GPU, e avaliar de sua eficiência.
\\
...
Image registration is an essential step in a number of applications in image processing, 
from populational studies from medical imaging to map reconstruction based on satellite images. Non rigid deformations
represent a big slice of all possible deformations in images that go through the registration process. 
Presently studies that involve thousands of images are becoming very usual, 
suggesting that acceleration techniques should be developed to decrease the analysis or processing time. 
Registration techniques are usually very computational demanding, so the use of \textit{graphic processing units} (GPUs) 
creates a possibility to speedup several stages of the registration process, like the extraction of features, 
determination of feature correlation and the calculate of the deformation function. This project has the objective of 
studding and parallelizing non linear techniques applying then to an modern GPU architecture and evaluate theirs 
efficiency.
\\
